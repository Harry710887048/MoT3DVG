...
from .temporal import QueryDrivenTemporalFusion
...
class BeaUTyDETR(nn.Module):
    ...
    def __init__(
        ...
        # self.tempfusion = nn.Conv1d(d_model*5, d_model, kernel_size=1)
        # Text Encoder
        ...
        self.proposal_head = ClsAgnosticPredictHead(num_class, self.num_heading_bin, num_queries, d_model, objectness=False, heading=False, compute_sem_scores=True)
        ...
        for _ in range(self.num_decoder_layers):
            self.prediction_heads.append(ClsAgnosticPredictHead(num_class, self.num_heading_bin, num_queries, d_model, objectness=False, heading=False, compute_sem_scores=True))
        ...
        self.temp_attn = QueryDrivenTemporalFusion(d_model, window_size_t=3)
    ...
    def _run_backbones(self, inputs):
        ...
        # Visual encoder
        pcd_multi = inputs["point_clouds_list"]
        B, NF, N, C = pcd_multi.shape
        pcd_multi = pcd_multi.view(B*NF, N, C)
        with torch.no_grad():    
            end_points_multi = self.backbone_net(pcd_multi)
        
        end_points = self.backbone_net(inputs["point_clouds"][:, :, :3])
        end_points["seed_inds"] = end_points["fp2_inds"]
        end_points["seed_xyz"] = end_points["fp2_xyz"]
        end_points["seed_features"] = end_points["fp2_features"]
        
        feat_multi = end_points_multi["fp2_features"].view(B, NF, -1, end_points_multi["fp2_features"].shape[-1])
        enhanced_feats = self.temp_attn(
            f_hist=feat_multi,
            f_curr=end_points["fp2_features"],
            f_hist_last=feat_multi[:, -1, :, :],
        )
        end_points["fp2_features"] = enhanced_feats
    ...
    def forward(self, inputs):
        ...
        cluster_feature = end_points["query_points_feature"]  # (B, F, V=256)
