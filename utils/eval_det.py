...
def iou3d_rotated_vs_aligned(gt_bboxes_rotated, pred_bboxes_aligned):
    ...
    Returns:
        ...
        center_distances: (M, N) - Euclidean distances between GT and pred bbox centers
    """
    ...
    # Extract bbox centers for distance calculation
    gt_centers = gt_bboxes_rotated[:, :3]
    pred_centers = pred_bboxes_aligned[:, :3]

    center_distances = torch.norm(
        gt_centers.unsqueeze(1) - pred_centers.unsqueeze(0), 
        dim=-1
    )  # (M, N)
    ...
    return ious, union, center_distances
